...
export async function GET(req: NextRequest) {
    try {
        ...
        const response = await fetch(`${INTERNAL_API_URL}/api/config`, {
            headers: {
        ...
        if (!response.ok) {
            ...
            console.error(`Internal API returned ${response.status} for /api/config: ${errorText}`);
            return NextResponse.json(
        ...
        console.error('Proxy error for /api/config:', error);
        return NextResponse.json(
