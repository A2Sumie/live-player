...
export async function POST(req: NextRequest) {
    try {
        ...
        // Optional: Check if user is admin if role exists
        ...
        const response = await fetch(`${INTERNAL_API_URL}/api/server/restart`, {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            console.error(`Internal API returned ${response.status} for /api/server/restart: ${errorText}`);
            return NextResponse.json(
        ...
        console.error('Proxy error for /api/server/restart:', error);
        return NextResponse.json(
